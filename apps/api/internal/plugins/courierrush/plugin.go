...
// (Pizza-Taxi style bike delivery run). It exists to prove the kernel.GamePlugin
// contract end-to-end and is the template for future plugins.
package courierrush
...
func init() {
	kernel.DefaultRegistry.Register(&Plugin{})
}

type Plugin struct{}

func (p *Plugin) Metadata() kernel.PluginMetadata {
...
func (p *Plugin) StartSession(_ context.Context, _ kernel.SessionConfig) (string, error) {
	buf := make([]byte, 8)
...
func (p *Plugin) ProcessInput(_ context.Context, _ string, _ kernel.GameInputEvent) error {
	return nil
}
...
func (p *Plugin) ComputeScore(_ context.Context, _ string, raw []byte) (kernel.GameSessionResult, error) {
	var fp finalPayload
	if err := json.Unmarshal(raw, &fp); err != nil {
		return kernel.GameSessionResult{}, fmt.Errorf("courierrush: invalid payload: %w", err)
	}
	if fp.DurationSec <= 0 || fp.DurationSec > maxDurationSec {
		return kernel.GameSessionResult{}, fmt.Errorf("courierrush: duration %v out of bounds", fp.DurationSec)
	}
	if fp.Deliveries < 0 || fp.Crashes < 0 || fp.ComboMax < 0 {
		return kernel.GameSessionResult{}, fmt.Errorf("courierrush: negative counters")
	}
	maxPlausibleDeliveries := int(math.Floor(fp.DurationSec/minSecondsPerDelivery)) + 1
	if fp.Deliveries > maxPlausibleDeliveries {
		return kernel.GameSessionResult{}, fmt.Errorf("courierrush: %d deliveries implausible for %.1fs", fp.Deliveries, fp.DurationSec)
	}

	score := max(int64(fp.Deliveries*100+fp.ComboMax*20)-int64(fp.Crashes*15), 0)
	completed := fp.Deliveries >= completionThreshold

	rewards := kernel.ResourceGrant{
		CashDelta:   fp.Deliveries * 8,
		TrustDelta:  boolToInt(completed)*5 + fp.Deliveries,
		EnergyDelta: -5,
	}

	return kernel.GameSessionResult{
		Score:          score,
		Completed:      completed,
