...
| **M30** | **Hi-Fi Procedural Skin Plugin Architecture** | A direct request to "replan the whole style" led to auditing the skin system end-to-end: every one of the 5 existing skins (including `retro_gb`) is palette-JSON-only — no real texture files anywhere, and the actual pixels come from exactly 3 hardcoded functions in `WorldScene.ts` that only the tileset one even reads the palette from (hero/NPC art is byte-identical across every skin). A real hi-fi skin needs new drawing *code*, not new data, so this plans a new `SkinRenderer` plugin contract (mirroring M29's `MinigameModule`/`MinigameLoader` exactly) plus 3 new procedural hi-fi skins — Diorama Glow, Flat Vector Minimal, Neon Night-City — each shipped as its own dynamically-`import()`ed bundle rather than compiled into the main app bundle | `[ ] Planned` — not yet implemented; see EPIC-30 non-goals (no external art, no WebGL/Light2D, no sandboxing for future community renderer code) | [EPIC-30](stories/EPIC-30-hifi-skin-plugin-architecture.md) | [M30 tasks](tasks/M30-hifi-skin-plugin-architecture.md) |
