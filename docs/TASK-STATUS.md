...
Last updated: 2026-09-18 (M30 implemented)

## Status Log

- **2026-09-15 — Full repo audit** (see [`docs/AUDIT-2026-09-15.md`](file:///Users/rex-fab-alt/Documents/private/District_Common-Ground/docs/AUDIT-2026-09-15.md)) and same-day close-out sprint (see [`docs/SPRINT-2026-09-15-PLAN.md`](file:///Users/rex-fab-alt/Documents/private/District_Common-Ground/docs/SPRINT-2026-09-15-PLAN.md)) closed every tracked gap:
  - M12, M15, M16, M17, M18, M19 audit-verified genuinely complete and archived.
  - M11's hollow Global Solidarity Pool and missing Safe Haven ending made real (real write path, proven end-to-end + real ending trigger).
  - M9's Broadsheet headline made genuinely AI-driven with a citation pill; the Go/frontend archetype-naming mismatch fixed; a radio ticker added. Real RSS ingestion stays formally deferred (a scope decision, not a gap).
  - M14's 3 missing QA tests built; M20's 3 concrete gaps (scripts, remote loading, naming) closed — both archived too, alongside the six original audit-clean milestones.
  - M8 formally re-scoped as a fully synthetic economy (`Wage`/`Transit` fixed from a permanent no-op to real seasonal curves).
  - M10's EPIC-10 Weather System vision built (frost/rain canvas layers + a tested pure classifier).
  - Every milestone through M20 is `[x] Complete` — see "Post-Audit Follow-ups" below for the full resolution history.
- **2026-09-15 — M21 (Visual Overhaul & Cozy Art Direction)** planned and implemented same-day, from `docs/kickstart/Visual Overhaul & Skin Design Specification.md`, grounded against the real current code (camera zoom, tileset colors, skin manifest schema, resilience-tier CSS, dialogue/radio widgets). All 10 task-doc sections code-complete (`tsc --noEmit` clean, `vitest run` 337/337, `oxlint` clean); Tests 21.6/21.7 stay manual (need a live browser).
- **2026-09-15 — M22 (UI Skin Kit & "Modern Shine" Redesign)** planned and implemented same-day, from a direct request to move away from the pixel-art "Zelda" look. Sections 1–3 (skin-system-reaches-the-UI foundation, the new `SkinUIKit` schema, the "Aurora Glass" proof skin) code-complete (`tsc --noEmit` clean, `vitest run` 341/341, `oxlint` clean, `go build`/`go test -race -short` clean); Section 4 (full 25-modal sweep) deferred by design, Tests 22.4/22.5 stay manual.
- **2026-09-15 — M13 (Playtesting, Balancing & Telemetry)**, surfaced by a follow-up "what's open in our visions" audit as a real bookkeeping gap (a planning doc that existed since early in the project but was never given a task doc, story epic, or table row) — implemented the same day: a deterministic 90-day solvency sweep (the spec's literal "10,000 Monte Carlo runs" doesn't fit a codebase with zero randomness in its economy math), three zero-PII telemetry events riding the existing M18 signed event log, a new economic-attrition-rate backend surface, and a per-scenario solidarity breakdown. `tsc`/`vitest` (352/352)/`oxlint`/`go build`/`go vet`/`go test -race -short` all clean, plus the full Docker/testcontainers integration suite against real Postgres.
- **2026-09-16 — M23 (Content Variety & Delight)**, from a direct user complaint ("always the same messages... very sad and repeating"): a code audit found dialogue/gossip/quests hard-cycling a tiny fixed content set forever, a real bug where crises stop triggering once the scenario pool drains once, uniformly grim tone, and no celebratory feedback on the core loop's positive beats. All 6 fixes shipped same day: a shared crisis-queue reshuffle helper, `TactileEffects` wired into Commons completion + solidarity choices, dialogue grown 3→5 trees/NPC, a second day-alternating gossip variant per archetype, quests grown 3→6 with day-based rotation, and a second BGM chord progression gated on day/night. `tsc --noEmit` clean, `vitest run` 371/371, `oxlint` clean, `go build`/`go test -race -short` clean.
- **2026-09-16 — M24 (Economy & Energy Rebalance)**, surfaced first out of a 4-part grounding investigation into a same-day follow-up request (multiplayer trading, more NPCs, more minigames, an earn-money/energy + real-news-feed audit): there was no repeatable player-driven earn action at all, the design-intended inflation→energy multiplier was a documented no-op, and one quest granted a wildly oversized energy refill. Fixed same day: `applyDailyTick()` now reads `multipliers.energy` for upkeep, each archetype got a once-per-day player-initiated "Work" action trading energy for cash, and the oversized quest reward was rebalanced to a capped +25. `tsc --noEmit` clean, `vitest run` 383/383, `oxlint` clean, `go build`/`go test -race -short` clean.
- **2026-09-17 — M25 (Multiplayer Trading Between Neighbors)**, same investigation: the friends graph only supported one-directional gifting (no acceptance step) — added a real propose/accept/decline/cancel trade-offer flow (new `trade_offers` table, 7 new Go endpoints, a new "Trade" tab in the Social Hub) reusing the caravan system's "server gates+records, client owns resources" trust model, plus auto-settle-on-load. `tsc`/`vitest` (383/383)/`oxlint`/`go build` all clean; `go test -race` on `internal/social` ran for real against Postgres via Docker — all pass, including the pre-existing friend/caravan suite (no regressions).
- **2026-09-17 — M26 (NPC Roster Expansion)**, same investigation: the live roster was exactly 3 NPCs (Mira, Leo, Elena), leaving 2 of 5 construction nodes with no NPC voice — added Sal, Marcus, and Mrs. Higgins from the pre-existing "Seven Pillars" character bible (`docs/planning/09-NPC-SOCIAL-NETWORK-AND-RELATIONSHIPS.md`), each wired through every system the original 3 touch (5-tree rotating dialogue, daily gossip, skin-manifest tokens, procedural NPC textures). `tsc --noEmit`/`oxlint` clean, `vitest run` 384/384 (no regressions); no backend changes needed.
- **2026-09-17 — M27 (Minigame Expansion)**, same investigation, on a direct follow-up ("please create more minigames! make sure they are outstanding nice!"): the M14 microkernel's own `MinigameCategory` contract names 5 categories but only `delivery` (Courier Rush) was built — added the remaining 4 as new standalone packages (Tenant Rights Match, Community Kitchen Rush, Solidarity Line, Tool Library Workshop) with real particle effects, combo scoring, and juicy feedback, wired through `MinigameLoader` and new in-world portals. `tsc --noEmit` (apps/web + all 4 packages standalone) and `oxlint` clean, `vitest run` 384/384 (no regressions); no backend changes needed. Real-news-feed integration remained the last item from that investigation.
- **2026-09-17 — M28 ("2026 Eye-Candy" Audit & Stability Fixes)**: a live re-test of a prior undocumented visual overhaul surfaced 8 issues in one message. Root cause for the two dominant complaints (flicker + muted colors): `#ui-root` was nested *inside* `#game-container`'s resilience-tier CSS filter, which ran an unconditional infinite pulse animation from a starting `resilienceScore` of `0` on every fresh game. Fixed the DOM nesting, the infinite animation, and the starting score; fixed a double-`JustDown()` keyboard bug that left `E` dead for every world interaction; made the NPC/node speech bubble actually clickable; added consistent Escape-to-close (+ 3 listener-leak fixes) across 14 modals; hid keyboard hints on touchscreens; finished the icon-toolbar/`DistrictGrid` skin-color migration; gave the main-menu Geo-Mode preview its own visual identity. `tsc --noEmit` clean, `vitest run` 384/384 (no regressions), `oxlint` clean; live-verified against the running dev stack. Minigame-plugin-architecture routing deliberately deferred to M29.
- **2026-09-17 — M29 (Route Built-in Minigames Through the Plugin/Remote-Load System)**, tackling that deferred item directly: the remote-loading path (`MinigameLoader.loadRemoteMinigame`) already existed and was already tested, but dead in production — no minigame package had a real build step and each manifest's `entrypointUrl` was a bare npm specifier a real browser `import()` can't resolve. Gave each package a real Vite library-mode build emitting a standalone ESM bundle to `apps/web/public/plugins/<id>/index.js` (wired into `make build`/`make dev`/CI); replaced `main.ts`'s 5 static imports with a hardcoded fallback catalog merging a live `GET /api/v1/games` fetch; fixed a real pre-existing Go field-shape bug (`PluginMetadata`'s JSON tags didn't match the frontend's `title`/`entrypointUrl`). `tsc --noEmit` clean, `vitest run` 388/388 (4 new, no regressions), `oxlint` clean, `go build`/`go vet`/`go test -race` (short + full Docker integration) all clean; live-verified with a production build proving zero minigame gameplay code in the main bundle, plus an API-down check proving the offline fallback actually works.
- **2026-09-18 — M30 (Hi-Fi Procedural Skin Plugin Architecture)**, from a direct request to "replan the whole style of the whole game": audited the skin system end-to-end and found every skin (including `retro_gb`) was palette-JSON-only with zero real art anywhere — the actual pixels came from exactly 3 hardcoded `WorldScene.ts` functions, byte-identical across every skin for hero/NPC art. Built a `SkinRenderer` plugin contract + `SkinRendererLoader` (mirroring M29's `MinigameModule`/`MinigameLoader` pattern exactly) and 3 new procedural hi-fi skins — **Diorama Glow** (lit-gradient tiles, soft drop-shadows, rounded cel-shaded sprites), **Flat Vector Minimal** (flat color bands + one precomputed noise pattern, bold capsule sprites), **Neon Night-City** (near-black grounds, layered-alpha glow-stroke edges, angular rim-lit sprites) — each its own standalone `packages/skin-*` package, dynamically `import()`ed from `/plugins/skins/<id>/index.js`, never compiled into the main bundle. Retired the dead M5-era "Cozy Vector" locked placeholder in `SettingsModal.ts`. `tsc --noEmit` clean, `vitest run` 391/391 (3 new), `oxlint` clean, `go build`/`go vet`/`go test -race -short` clean; live-verified (`GET /api/v1/themes` lists all 8 skins, all 3 bundles self-contained and serving correctly). No headless browser was available in this environment for the live "switch skin, see the redraw" screenshot check — that stays manual, same as this project's other WorldScene visual tests.
...
| **M28** | **"2026 Eye-Candy" Audit & Stability Fixes** | A live re-test of a prior (undocumented) visual overhaul surfaced 8 concrete issues — flickering/muted colors, dialogs missing a close, main-menu/in-game visual bleed, dead keyboard interactions, a non-clickable speech bubble, keyboard hints needing to hide on mobile, and minigames not going through the plugin system. Root-caused via 3 parallel code audits: `#ui-root` was nested inside `#game-container`'s resilience-tier CSS filter (an infinite-pulse animation active by default on every fresh game) — this was both the flicker and the muted-HUD-color complaint at once. Fixed the DOM nesting, the infinite animation, and the fresh-game starting resilience score; fixed a deterministic double-`JustDown()` bug that made `E` dead for every world interaction; made the NPC/node bubble actually clickable; added consistent Escape-to-close (+ fixed 3 listener leaks) across 14 modals via one shared helper; hid keyboard hints on touchscreens; finished the icon-toolbar/DistrictGrid/`.settings-panel` skin-color migration; gave the main-menu Geo-Mode preview its own visual identity | `[x] Implemented` — `tsc --noEmit` clean, `vitest run` 384/384 (no regressions), `oxlint` clean; live-verified against the running dev stack. Minigame-plugin-architecture routing formally deferred (see EPIC-28 non-goals); no backend changes needed | [EPIC-28](stories/EPIC-28-eye-candy-audit-and-stability-fixes.md) | [M28 tasks](tasks/M28-eye-candy-audit-and-stability-fixes.md) |
| **M29** | **Route Built-in Minigames Through the Plugin/Remote-Load System** | Tackled M28's deferred item #8 directly. `MinigameLoader.loadRemoteMinigame` already existed and was already tested, but dead in production — the 5 built-in games were still statically imported into the main bundle, since no package had a real build step and each manifest's `entrypointUrl` was a bare npm specifier no real browser `import()` could resolve. Gave each package a real Vite library-mode build emitting a standalone ESM bundle to `apps/web/public/plugins/<id>/index.js` (wired into `make build`/`make dev`/CI); replaced `main.ts`'s 5 static imports with a hardcoded fallback catalog that merges with a live `GET /api/v1/games` fetch; fixed a real pre-existing Go field-shape bug (`PluginMetadata`'s JSON tags didn't match the frontend's `title`/`entrypointUrl`, so even courier-rush's catalog entry rendered blank) and added metadata-only catalog entries for the other 4 games | `[x] Implemented` — `tsc --noEmit` clean, `vitest run` 388/388 (4 new, no regressions), `oxlint` clean, `go build`/`go vet`/`go test -race` (short + full Docker integration) all clean; live-verified with a production build proving zero minigame code remains in the main bundle, plus an API-down check proving the offline fallback actually works. Sandboxed third-party-plugin routing and real backend anti-cheat for the 4 non-courier games formally deferred (see EPIC-29 non-goals) | [EPIC-29](stories/EPIC-29-minigame-plugin-architecture.md) | [M29 tasks](tasks/M29-minigame-plugin-architecture.md) |
| **M30** | **Hi-Fi Procedural Skin Plugin Architecture** | A direct request to "replan the whole style" led to auditing the skin system end-to-end: every one of the 5 existing skins (including `retro_gb`) is palette-JSON-only — no real texture files anywhere, and the actual pixels came from exactly 3 hardcoded functions in `WorldScene.ts` that only the tileset one even read the palette from (hero/NPC art was byte-identical across every skin). Built a new `SkinRenderer` plugin contract + `SkinRendererLoader` (mirroring M29's `MinigameModule`/`MinigameLoader` exactly, `rendererUrl?` on `SkinManifest`) plus 3 new procedural hi-fi skins — **Diorama Glow** (lit-gradient tiles, soft drop-shadows, rounded cel-shaded sprites), **Flat Vector Minimal** (flat color bands + one precomputed noise pattern, bold capsule sprites), **Neon Night-City** (near-black grounds, layered-alpha glow-stroke edges, angular rim-lit sprites) — each its own standalone `packages/skin-*` package, shipped as a dynamically-`import()`ed bundle at `/plugins/skins/<id>/index.js`, never compiled into the main app bundle. Retired the dead M5-era "Cozy Vector" locked placeholder card in `SettingsModal.ts` in favor of the 3 real skins | `[x] Implemented` — `tsc --noEmit` clean, `vitest run` 391/391 (3 new: `SkinRendererLoader.test.ts`), `oxlint` clean, `go build`/`go vet`/`go test -race -short` clean; live-verified against the running dev stack (`GET /api/v1/themes` lists all 8 skins, all 3 renderer bundles build self-contained with zero residual workspace imports and serve correctly). No headless browser was available to take the live "switch skin and see the redraw" screenshots — that stays manual, same as this project's existing WorldScene visual tests. See EPIC-30 non-goals (no external art, no WebGL/Light2D, no sandboxing for future community renderer code — a real open risk, flagged not solved) | [EPIC-30](stories/EPIC-30-hifi-skin-plugin-architecture.md) | [M30 tasks](tasks/M30-hifi-skin-plugin-architecture.md) |
