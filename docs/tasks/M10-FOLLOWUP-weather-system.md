...
Status: `[x] Resolved` — Option 1 (build it) chosen 2026-09-15; see `docs/SPRINT-2026-09-15-PLAN.md` §6.
...
- [x] Decided Option 1 (build it).
- [x] Rain/frost render as separate canvas layers (frost tint at depth 91, rain drops at depth 92) above the day/night tint (depth 90) — they compose because each is its own render layer, not competing `filter:` rules on the same element, which was the actual risk worth calling out here.
- [x] `WeatherSystem.test.ts` covers the pure `weatherTier(heat)` threshold logic across the full `[0.4, 1.5]` range `economy.go` produces — the Phaser rendering itself isn't chased down for automated coverage, mirroring how `resilienceTier()` is tested.
- [x] `npm test` (319/319) passes with no regressions.
