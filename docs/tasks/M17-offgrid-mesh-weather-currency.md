...
- [x] `manifest.json` — plugin configuration requesting `geo:read` permission (also declares `weather:sync`)
- [x] `SunCalcEngine.ts`:
...
- [x] `manifest.json` — permissions for `serial:connect`, `bluetooth:connect`
...
- [x] Serverless P2P Comms:
  ...
  - [x] `MeshChatModal.ts` — in-game walkie-talkie & emergency broadcast terminal, transport picker (Serial/Bluetooth/WebRTC) + composer + received-packet log, wired into `TopHUD.ts`

---

## 3. Decentralized Mutual Credit & Local Ledger Plugin (`apps/web/src/plugins/mutual-credit/`)
- [x] `manifest.json` — permissions for `crypto:sign`, `wallet:ledger`
...
- [x] **Test 17.1 (Solar Day/Night Sync):** `apps/web/src/plugins/geo-weather/WeatherModifierBridge.test.ts` — real `getSolarState()` at 22:00, 52.5°N in winter reports `isNight: true`, and the resulting `solarEfficiencyMult` is 0 (streetlamp activation is represented by `isNight`, not a live sprite toggle — see scoping note 2).
- [x] **Test 17.2 (Live Weather Modifiers):** `apps/web/src/plugins/geo-weather/WeatherModifierBridge.test.ts` — a -5°C / 4.2mm-rain reading increases `energyHeatingUpkeep` above 1 and sets `gardenWaterBonus: true`.
- [x] **Test 17.3 (LoRa Serial Codec):** `apps/web/src/plugins/mesh-comms/PacketCodec.test.ts` — a 20 kW caravan dispatch packet encodes under 200 bytes (well inside the 237-byte LoRa ceiling) and decodes back to an identical object.
- [x] **Test 17.4 (Zero-Server P2P Transaction):** `apps/web/src/plugins/mutual-credit/CryptoLedger.test.ts` — a 15 ST transaction is signed offline with a locally generated Ed25519 keypair, its signature validates, and `fetch` is never called during signing or verification.
